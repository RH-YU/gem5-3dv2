...
`run-rvv-npu-vcu-smoke` 验证标准 RVV 编码进入 NPU VCU 后端后的 VADD 数据流，`run-rvv-npu-backpressure` 验证 RVV NPU VCU FIFO 背压观察，`run-multinpu` 验证单 CPU 向多个 NPU 广播命令并产生各自 GM 输出，`run-cube-smoke` 验证 GM/L1/L0/Cube/Fixpipe/UB/GM 数据通路，`run-niu-smoke` 验证跨 NPU UB/GM 搬运和 remote sync token 通过 NIU/NOC 传递。
